...
class VekSpider(Spider):
    ...
    start_urls = ['https://www.21vek.by/notebooks',
                  'https://www.21vek.by/tv']
    ...
    def parse_pages(self, response, **kwargs):
        max_page = int(response.xpath("//div[@id='j-paginator']//a[last()-1]/@name").get())
    ...
    def parse(self, response, **kwargs):
        for product in response.xpath("//ul[@class='b-result']/li"):
            yield ProductItem(
                ...
                code=product.css("span.g-code::text").get(),
